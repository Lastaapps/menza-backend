...
class RatingRepositoryImpl(
    ...
    override suspend fun rate(id: String, rating: UInt): Result<Unit> {
        ...
        map.update {
            it.put(id, it.getOrDefault(id, DishProgress.default).run {
    ...
    override fun getState(): Flow<List<DishStatus>> = map.map { map ->
        map.map { (id, progress) ->
            with(progress) {
                DishStatus(
                    id,
